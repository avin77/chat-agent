...
must_haves:
  truths:
    ...
    - "KPI cards: lead completion rate, lead quality score, effective escalation rate, avg session duration"
    ...
    - "checkAndWriteAlerts() evaluates all 4 alert types and inserts to system_alerts"
  artifacts:
    ...
    - path: "src/app/dashboard/page.tsx"
      provides: "Product Health tab render block with all 5 sections"
      contains: "activeTab === 'product_health'"
  key_links:
    - from: "src/app/dashboard/page.tsx"
      to: "src/app/dashboard/actions.ts"
      via: "getTokenCostMetrics(), getShadowMetrics(), getSystemAlerts() imported and called in fetchAll()"
      pattern: "getTokenCostMetrics"
    - from: "src/app/dashboard/page.tsx"
...
<objective>
Complete the Product Health dashboard tab with all metrics sections, and add the four new server actions that power them.

Purpose: This is the primary user-facing deliverable of Phase 3. The tab exists but renders nothing. This plan adds all the data actions and JSX rendering for token costs, shadow alignment, and system alerts.

Output:
- `src/app/dashboard/actions.ts` — 4 new exports + extended getProductHealthMetrics()
- `src/app/dashboard/page.tsx` — complete Product Health tab render block
</objective>
...
export async function checkAndWriteAlerts() {
    ...
    // ALERT-04: Shadow alignment < 95% (only if we have enough data)
...
```
  ...
  <done>
    ...
    checkAndWriteAlerts() exported from actions.ts — checks all 4 alert types.
...
**Step 3: Add new fetches to fetchAll().**

In `fetchAll()`, extend the `Promise.all` array to include the 3 new fetches. Current call is:
```typescript
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="space-y-1 text-sm text-gray-600">
                    {[
                        ...
                        { label: 'No cost anomaly (shadow avg < 2× prod avg)', pass: true }, // manual check
                        { label: 'No repeated invalid tool proposals (> 3× same wrong tool/day)', pass: true }, // manual check
...
Insert this block directly before the closing `</div>` at the end of the outer content wrapper (the one that wraps all tab content, around line 913).
  ...
  <done>
    ...
    activeTab === 'product_health' render block present with all 5 sections: alerts banner, KPI cards, slot fill rates, token cost, shadow panel.
    Shadow panel includes gate conditions checklist.
  </done>
</task>
...
Product Health tab has content: KPI cards, slot fill rates table, token cost section, shadow alignment panel, additional health metrics.
All 5 new server actions exported from actions.ts: getTokenCostMetrics, getShadowMetrics, getSystemAlerts, checkAndWriteAlerts.
getProductHealthMetrics() returns fieldStats (filled/skipped/total per field) and p50SessionDurationMs.
Alert system checks all 4 thresholds: fallback rate, LLM error rate, eval regression, shadow alignment.
</success_criteria>
